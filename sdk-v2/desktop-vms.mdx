...
<Note>
`session.evaluate()` requires a linked testcase. This example uses raw artifact IDs, so there's nothing to score against — it calls `session.get_state()` to flush and inspect mutations instead. To run evaluation, create the session from a testcase with `plato.sessions.create(testcase="<test-case-id>")` — see the [testcase variant](#testcase-variant-with-evaluate) below.
</Note>
...
|-----|------|-------------|
| `publicId` | `str` | The testcase ID you pass to `sessions.create(testcase=...)` |
| `name` | `str` | Human-readable task name |
| `prompt` | `str` | The task instructions to give to the agent |
| `scoringTypes` | `list[str]` | How the task is scored (`"MUTATION"`, `"OUTPUT"`, or both) |
| `outputSchema` | `dict \| None` | Required JSON schema when scoring type includes `OUTPUT` |
| `startUrl` | `str \| None` | Optional URL the agent should navigate to first |
